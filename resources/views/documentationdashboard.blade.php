...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .donut { width: 140px; height: 140px; border-radius: 999px; margin: 0 auto 8px; background: conic-gradient(#8b5cf6 0 34%, #22d3ee 34% 60%, #60a5fa 60% 82%, #f472b6 82% 100%); display: grid; place-items: center; }
...
@php
    $docStats = [
        ...
        'processing_documents' => \App\Models\Document::where('status', 'processing')->count(),
        'completed_documents' => \App\Models\Document::where('status', 'completed')->count(),
        'total_transfers' => \App\Models\TitleTransfer::count(),
        'pending_transfers' => \App\Models\TitleTransfer::where('status', 'pending')->count(),
    ];
    $sidebarModules = [
...
<div class="shell">
    ...
    <main class="main">
        <header class="header">
            <div class="h-brand">
                <div class="h-title">Documentation Dashboard</div>
                <div class="h-subtitle">Welcome, {{ auth()->user()->name ?? 'Documentation' }}</div>
            </div>
            ...
            <div class="header-actions" style="display:flex; align-items:center; gap:16px;">
                ...
                <!-- Dark Mode Toggle -->
                <button type="button" id="themeToggle" title="Toggle Dark Mode" style="border:1px solid var(--border-color); background:var(--bg-card); border-radius:10px; width:34px; height:34px; display:inline-flex; align-items:center; justify-content:center; color:var(--text-secondary); cursor:pointer;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M6 .278a.768.768 0 0 1 .08.858 7.208 7.208 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277.527 0 1.04-.055 1.533-.16a.787.787 0 0 1 .81.316.733.733 0 0 1-.031.893A8.349 8.349 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.752.752 0 0 1 6 .278z"/></svg>
                </button>

                <!-- Profile Dropdown -->
                <div id="profileDropdownBtn" style="position:relative; display:flex; align-items:center; gap:8px; cursor:pointer; background:var(--bg-card); border:1px solid var(--border-color); padding:4px 8px; border-radius:12px;">
                    @php
                        $userName = auth()->user()->name ?? 'Documentation';
                        $userInitial = strtoupper(substr($userName, 0, 1));
                    @endphp
                    <div style="width:26px; height:26px; border-radius:50%; background:linear-gradient(135deg, #6366f1, #4f46e5); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:12px;">
                        {{ $userInitial }}
                    </div>
                    ...
                    <!-- Dropdown Menu -->
                    <div id="profileMenu" style="position:absolute; top:45px; right:0; width:160px; background:var(--bg-card); border:1px solid var(--border-color); border-radius:12px; box-shadow:0 10px 25px -5px rgba(15,23,42,0.1); padding:8px 0; display:none; flex-direction:column; z-index:100;">
        ...
        <section class="content">
            <div class="summary">
                <div class="card c-teal"><div class="k-label">Documents</div><div class="k-number">{{ $docStats['total_documents'] }}</div><div class="k-sub">Pending {{ $docStats['pending_documents'] }} | Processing {{ $docStats['processing_documents'] }}</div></div>
                <div class="card c-blue"><div class="k-label">Completed Docs</div><div class="k-number">{{ $docStats['completed_documents'] }}</div><div class="k-sub">Processed legal requirements</div></div>
                <div class="card c-pink"><div class="k-label">Title Transfers</div><div class="k-number">{{ $docStats['total_transfers'] }}</div><div class="k-sub">Pending {{ $docStats['pending_transfers'] }}</div></div>
                <div class="card c-purple"><div class="k-label">Compliance</div><div class="k-number">On Track</div><div class="k-sub">BIR/CAR/ROD milestone monitor</div></div>
            </div>

            <div class="middle">
                <div class="panel">
                    <div class="panel-head"><div class="panel-title">Processing Trend</div><div class="chip">Last 6 months</div></div>
                    <div class="line-wrap">
                        <svg viewBox="0 0 600 220" preserveAspectRatio="none">
                            <defs><linearGradient id="docFillLine" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stop-color="#93c5fd" stop-opacity="0.55"></stop><stop offset="100%" stop-color="#93c5fd" stop-opacity="0"></stop></linearGradient></defs>
                            <path d="M0,196 C58,172 98,178 146,148 C188,122 236,158 286,126 C332,98 370,114 426,93 C476,74 520,86 600,64 L600,220 L0,220 Z" fill="url(#docFillLine)"></path>
                            <path d="M0,196 C58,172 98,178 146,148 C188,122 236,158 286,126 C332,98 370,114 426,93 C476,74 520,86 600,64" fill="none" stroke="#60a5fa" stroke-width="4"></path>
                        </svg>
                    </div>
                </div>
                <div class="panel">
                    <div class="panel-head"><div class="panel-title">Document Status Split</div><div class="chip">This month</div></div>
                    <div class="donut"></div>
                    <div class="legend">
                        <div><span>Pending</span><strong>34%</strong></div>
                        <div><span>Processing</span><strong>26%</strong></div>
                        <div><span>Completed</span><strong>24%</strong></div>
                        <div><span>Transfer Steps</span><strong>16%</strong></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bottom">
                <div class="panel">
                    <div class="panel-title">Documentation Goals</div>
                    <div class="goal"><div class="goal-row"><span>Contract Completion</span><strong>64%</strong></div><div class="track"><div class="fill" style="width:64%"></div></div></div>
                    <div class="goal"><div class="goal-row"><span>Title Transfer Milestones</span><strong>51%</strong></div><div class="track"><div class="fill" style="width:51%"></div></div></div>
                    <div class="goal"><div class="goal-row"><span>BIR Processing Speed</span><strong>58%</strong></div><div class="track"><div class="fill" style="width:58%"></div></div></div>
                </div>
                <div class="panel">
                    <div class="panel-title">Quick Summary / Notes</div>
                    <div class="notes">
                        <div class="note">Pending documents require follow-up this week.</div>
                        <div class="note">Coordinate title transfer submission batches.</div>
                        <div class="note">CAR issuance timeline should be monitored closely.</div>
                        <div class="note">Maintain complete records for audit readiness.</div>
                    </div>
                </div>
            </div>
        </section>
